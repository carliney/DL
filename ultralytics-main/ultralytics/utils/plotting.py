...
class Annotator:
    ...
    def te_number(self,a,b):
        ...
        te_lines = [line.format(a[0], a[1], b[0], b[1], b[2], b[3], b[4]) for line in te_lines]
        line_height = 30
        y = 30
        for line in te_lines:
            cv2.putText(self.im, line, (2, y), 0, self.sf, (255, 255, 255), 2, cv2.LINE_AA)
            y += line_height
...
def plot_images(
    ...
    #annotator = Annotator(mosaic, line_width=4, font_size=10, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
